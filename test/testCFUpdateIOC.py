...
class UpdateIOCTest(ChannelFinderClientTestCase):
    ...
    def testGetArgsFromFilename(self):
        ...
        self.assertTrue(
            hostname == "aaa" and iocname == "bbb",
            "failed to parse the file name correctly",
        )
        ...
        self.assertTrue(
            hostname == "aaa" and iocname == "bbb", "failed to parse the file correctly"
        )
        pass
    ...
    def testAddUpdateChannels(self):
        ...
        channels = client.find(
            property=[("hostName", hostName1), ("iocName", iocName1), ("time", t1)]
        )
        self.assertTrue(
            len(channels) == 2,
        ...
        # Cleanup
        client = ChannelFinderClient(
            BaseURL=self.baseURL, username=self.username, password=self.password
        )
    ...
    def testPreservingOfAttributes(self):
        ...
        unaffectedTag = {"name": "unaffectedTag", "owner": self.owner}
        # create default client
        client = ChannelFinderClient(
            BaseURL=self.baseURL, username=self.username, password=self.password
        )
        ...
        for channel in channels:
            self.assertTrue(
                unaffectedProperty in channel["properties"]
        ...
        self.assertTrue(
            not client.find(property=[("iocName", "initialIoc")]),
            "Failed to cleanup old property",
        )
        # Case 4:
        updateChannelFinder(
    ...
    def __check4properties(self, prop, properties):
        ...
        """
        foundProp = [p for p in properties if p["name"] == prop["name"]]
        if len(foundProp) == 1 and foundProp[0]["value"] == prop["value"]:
            return True
        else:
            return False
    ...
    def testNoneCheck(self):
        ...
        self.assertTrue(ifNoneReturnDefault("", "default") == "default")
    ...
    def testPVUpdate(self):
        ...
        try:
            ...
            self.assertEqual(
                len(chs), 2, "Expected 2 positive matches but found " + str(len(chs))
            )
            updateChannelFinder(
                ["ch1"],
                "testHost",
            ...
            self.assertEqual(
                len(chs), 1, "Expected 1 positive matches but found " + str(len(chs))
            )
    ...
    def testPVMove(self):
        ...
        try:
            ...
            self.assertEqual(
                len(chs), 2, "Expected 1 positive matches but found " + str(len(chs))
            )
            self.assertEqual(
                client.find(property=[("hostName", "host1"), ("iocName", "ioc1")])[0][
                    "name"
                ],
                "ch1",
            ...
            self.assertEqual(
                client.find(property=[("hostName", "host1"), ("iocName", "ioc2")])[0][
                    "name"
                ],
                "ch2",
            ...
            self.assertEqual(
                len(client.find(property=[("hostName", "host1"), ("iocName", "ioc1")])),
                2,
            ...
            self.assertEqual(
                client.find(property=[("hostName", "host2"), ("iocName", "ioc2")])[0][
                    "name"
                ],
                "ch2",
                "Failed to find the expected channel _ch2_ with prop host1, ioc2",
    ...
    def testRegularExperssion(self):
        ...
        try:
            ...
            self.assertTrue(
                frozenset(pvNames).issuperset(frozenset(publicPVs)),
                "resulting pvNames contains invalid non public pvs",
            )
            self.assertTrue(
                frozenset(pvNames).isdisjoint(frozenset(privatePVS)),
