...
class ChannelUtilTest(unittest.TestCase):
    ...
    def testChannelUtil(self):
        ...
        channel2 = Channel(
            ...
            tags=[Tag("myTag", "tagOwner"), Tag("goldenOrbit", "tagOwner")],
        )
        channels = [channel1, channel2]
        allTags = ChannelUtil.getAllTags(channels)
        self.assertTrue(len(allTags) == 2, "expected 2 tags found " + str(len(allTags)))
        allPropertyNames = ChannelUtil.getAllProperties(channels)
        self.assertTrue(
            len(allPropertyNames) == 3,
    ...
    def testGetAllPropValues(self):
        ...
        self.assertTrue(
            "234" in values, "Failed to find property(location), value 234 for ch1"
        )
        self.assertTrue(
            "SR" in values, "Failed to find property(location), value SR for ch2"
        )
        self.assertTrue(
            "SR:234" in values,
    ...
    def testValidateWithTag(self):
        ...
        self.assertTrue(
            ChannelUtil.validateChannelsWithTag([ch1, ch2], Tag("myTag", "someOwner")),
            "Failed to validate channels based on TagValidator",
        )
        self.assertFalse(
            ChannelUtil.validateChannelsWithTag(
                [ch1, ch2], Tag("goldenOrbit", "someOwner")
            ),
    ...
    def testValidateWithProperty(self):
        ...
        self.assertTrue(
            ChannelUtil.validateChannelWithProperty(
                [ch2, ch3], Property("prop2", "anyOwner", "propVal")
            )
        )
        self.assertFalse(
            ChannelUtil.validateChannelWithProperty(
                [ch1, ch2, ch3], Property("prop2", "anyOwner", "propVal")
            ),
